...
- 主场城市会默认点亮；之后每次落地城市会继续**点亮**在暗色世界地图上，且**下一程必须从这里出发**——想点亮巴黎，就要规划一条真实可行的航线飞过去，留存动机由此产生；
...
| 机制 | 设计 | 为什么 |
|---|---|---|
| 压缩映射 | 按真实航程分段映射到 15/20/30/40/50/60… 友好档位 | 约 79% 航线落在 30~60 分钟，15 分钟只留给极短途 |
| 备降 | 离开 App 超 60 秒判"备降"；锁屏不算破戒 | 用航空术语替代"枯死/失败"的挫败叙事；不倒扣存量进度，惩罚克制 |
| 接力飞行 | 优选经济舱起可用；2 小时及以上一定可接力，部分 60~120 分钟航线可接力 | 跨洋航线可拆解，同时让"可接力"与">60 分"栏目保持差异 |
...
| 世界地图 | 每次进入自动定位当前城市，默认展示局部区域；顶部用统一"飞行总览"面板承载城市/国家/里程 | 强化当前位置感，同时避免统计数据割裂 |
...
- **世界地图完全自绘**（Natural Earth 轮廓 + SwiftUI Canvas，裁切极区的紧凑 Miller 圆柱投影 + 大圆航线插值）——对比谷歌地图方案：零 API 费用、完全离线、美术 100% 可控；
- **环形世界地图**：进入页面自动定位当前城市并放大到城市周边，经度方向无限循环，双指缩放始终保持当前地图中心；
- **地图视觉收敛**：裁切极区并压缩高纬面积，减少竖屏地图里南极/极地占比过大的问题；
- **全量中文检索**：3,691 个机场均有中文城市名，可按国家、城市、机场或 IATA 搜索，并区分锁定与无法直飞；
- **主场默认点亮**：新建档案时立即写入主场城市和护照盖章；老用户打开 App 时自动补齐；
...
选 iPhone Pro 系列模拟器运行即可预览灵动岛；免费 Apple ID 可真机安装自用（Personal Team 签名，7 天重签）。当前工程已在 Xcode 26.6 / iOS 26.5 Simulator 上验证。Xcode 26+ 首次运行需在 Settings → Components 里下载 iOS 模拟器运行时（约 8 GB）。

### 运行注意

- 正常调试请选择 `AirLine` scheme 运行主 App。
- `AirLineWidget` 只包含 Live Activity / Dynamic Island，不是普通桌面 Widget。不要直接运行 Widget target；在主 App 里开始一段飞行后，锁屏和灵动岛会由系统拉起。
- 若 Xcode 报 `Failed to get descriptors for extensionBundleID (app.airline.focus.widget)`，通常是直接 Show Widget 导致的，可以切回 `AirLine` scheme 运行。
...
```bash
# 预置档案 + 三段飞行历史（主场 PVG 默认点亮，飞行额外点亮 NRT/SIN/LHR）
